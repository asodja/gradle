...
use `'unnamed property'` without exposing their internal occurrence namespace.
This fallback does not claim a unique model location or a source variable name. Plugin IDs, plugin-class fallbacks and script origins
...
The measurements below describe the original D1 checkpoint, before the readable
anonymous-property label follow-ups. The latest change passed 26 diagnostic unit tests,
10 diagnostic integration tests and Java Checkstyle/Groovy CodeNarc checks. Integration
coverage verifies managed extension property names before and after copying/finalization.
The fallback is a string literal and adds no property fields or per-call label allocation.
The earlier allocation figures have not been remeasured for these changes;
the raw evidence and its source hashes remain historical and unchanged.
