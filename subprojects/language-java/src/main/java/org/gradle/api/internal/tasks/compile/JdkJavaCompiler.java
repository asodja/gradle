...
public class JdkJavaCompiler implements Compiler<JavaCompileSpec>, Serializable {
    ...
    private JavaCompiler.CompilationTask createCompileTask(JavaCompileSpec spec, JdkJavaCompilerResult result) {
        ...
        File classToFileMapping = compileOptions.getIncrementalCompilationClassToFile();
        File constantsMapping = compileOptions.getIncrementalCompilationClassToFileMapping();
        if (classToFileMapping != null && constantsMapping != null && compiler instanceof IncrementalCompilationAwareJavaCompiler) {
            task = ((IncrementalCompilationAwareJavaCompiler) compiler).makeIncremental(task, classToFileMapping, constantsMapping, new CompilationSourceDirs(spec.getSourceRoots()));
        }
