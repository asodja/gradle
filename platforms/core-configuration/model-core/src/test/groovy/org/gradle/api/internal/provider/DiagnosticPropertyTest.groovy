...
class DiagnosticPropertyTest extends Specification {
    ...
    def 'unnamed property reports a quoted fallback through #lifecycle while retaining occurrence identity'() {
        given:
        def namespace = '9672d53c-944c-4dce-8afe-b7b7c0c95a91/property/1'
        def anonymousHost = Stub(PropertyProvenanceHost) {
            getOwnerScope() >> scope
            newOccurrenceScope() >> namespace
            currentAttribution() >> attribution
        }
        def anonymous = new DiagnosticProperty<String>(anonymousHost, String)
        anonymous.set(Providers.notDefined())
        def occurrence = anonymous.lastAcceptedMutation
        ...
        'mutable'        | { it }
        'copy'           | { it.shallowCopy() }
        'finalized'      | { it.finalizeValue(); it }
        'finalized copy' | { it.finalizeValue(); it.shallowCopy() }
    }
