...
results = {
    ...
    'integrationSourceHashes': {name: hashlib.sha256((repo / 'platforms/core-configuration/model-core/src/main/java/org/gradle/api/internal/provider' / (name + '.java')).read_bytes()).hexdigest()
                                for name in ['DefaultPropertyFactory', 'AbstractProperty', 'CollectionPropertyDiagnostics', 'AbstractCollectionProperty', 'DefaultMapProperty', 'DiagnosticListProperty', 'DiagnosticSetProperty', 'DiagnosticMapProperty', 'CollectionPropertyProvenance', 'CollectionProvenanceSnapshot', 'PropertyUpdateClassifier', 'PropertyProvenanceDiagnostics']},
    'runs': [],
}
# Ordinary engines must not reference diagnostic integration, including observer contracts.
results['engineDiagnosticsBoundary'] = {}
for name in ['AbstractProperty', 'AbstractCollectionProperty', 'DefaultMapProperty']:
    bytecode = run(['javap', '-v', '-classpath', main, 'org.gradle.api.internal.provider.' + name])
    forbidden = ['org/gradle/api/internal/provenance/', 'CollectionPropertyProvenance',
                 'CollectionProvenanceSnapshot', 'PropertyProvenanceDiagnostics', 'PropertyProvenanceHost',
                 'DiagnosticListProperty', 'DiagnosticSetProperty', 'DiagnosticMapProperty',
                 'PropertyLifecycleObserver', 'CollectionPropertyDiagnostics']
    leaked = [reference for reference in forbidden if reference in bytecode]
    if leaked:
        raise RuntimeError(f'{name} references provenance implementation: {leaked}')
    results['engineDiagnosticsBoundary'][name] = 'no diagnostic or provenance references'
...
with tempfile.TemporaryDirectory(prefix='shared-provenance-probe-') as temporary:
    ...
    with zipfile.ZipFile(model_core_jar) as runtime:
        for name in ['DefaultListProperty', 'DefaultSetProperty', 'DefaultMapProperty', 'AbstractCollectionProperty', 'AbstractProperty']:
    ...
    run(['javac', '-cp', baseline_classpath, '-d', baseline_classes, source / 'ProvenanceAllocationProbe.java', source / 'CollectionBaselineProbe.java'])
    ...
    run(['javac', '-cp', new_classpath, '-d', metadata_classes, source / 'CollectionEnabledProbe.java'])
    ...
    for fork in range(3):
        for variant in variants[fork:] + variants[:fork]:
            ...
            entry = 'CollectionBaselineProbe'
            if variant == 'enabled':
                classpath = os.pathsep.join([str(metadata_classes), classpath])
                entry = 'CollectionEnabledProbe'
